...
RedisPubSub.prototype.setupListeners = function (cb) {
  ...
  this.subClient.on("message", function (channel, message) {
    if (self.listeners(channel).length > 0) {
      this.enableLogging && console.log("RedisPubSub:::", channel);
      this.enableLogging && console.log(JSON.stringify(self.listeners(channel)[0]));
...
RedisPubSub.prototype._subscribe = function(event, functino) {
  ...
  if (event != "removeListener" && event != "newListener") {
    this.enableLogging && console.log("RedisPubSub:::Subscribe", event, this.listeners(event).length);
    if (this.listeners(event).length == 0) {
      this.subClient.subscribe(event);
      this.enableLogging && console.log("RedisPubSub::subClient::subscribe::", event);
    }
  }
};
RedisPubSub.prototype._unsubscribe = function(event, functino) {
  ...
  if (event != "removeListener" && event != "newListener") {
    this.enableLogging && console.log("RedisPubSub:::Unsubscribe", event, this.listeners(event).length);
    if (this.listeners(event).length == 0) {
      this.subClient.unsubscribe(event);
      this.enableLogging && console.log("RedisPubSub::subClient::unsubscribe::", event);
    }
  }
};
